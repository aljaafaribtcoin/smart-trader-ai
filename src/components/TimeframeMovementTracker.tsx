  ...
  return (
    <Card className="p-6">
        ...
        {movements.map((movement) => (
          ...
          >
            <div className="flex items-center gap-3">
              <span className="font-mono font-semibold text-sm uppercase w-10">
                {movement.timeframe}
              </span>
              <MiniSparkline data={movement.sparklineData} />
            </div>
            <div className="text-left">
              <div className={`font-mono font-bold text-sm ${getChangeColor(movement.change)}`}>
      ...
      <div className="border-t border-border pt-4">
        <h4 className="text-sm font-semibold mb-3 text-muted-foreground">خريطة حرارية</h4>
        <div className="space-y-2">
          {movements.map((movement) => (
            <div key={movement.timeframe} className="flex items-center gap-2">
              ...
              <span className={`font-mono text-xs w-16 text-right ${getChangeColor(movement.change)}`}>
                {movement.change > 0 ? '+' : ''}{movement.change.toFixed(2)}%
              </span>
            </div>
          ))}
